...
def render_result(text):
    st.markdown(f'<div class="result-shell">{text}</div>', unsafe_allow_html=True)
...
Format: Q1. [Question] A) B) C) D) options, repeat for all, then add ✅ ANSWER KEY at the end."""
                result = ask(prompt)
                st.success("Generated Successfully ✅")
                render_result(result)
...
Structure: Header with instructions, Section A (MCQ/very short, ~20%), Section B (short answer, ~30%), Section C (long answer, ~50%).
Show marks per question in brackets. End with ✅ MODEL ANSWER KEY with brief correct points for every question."""
                result = ask(prompt)
